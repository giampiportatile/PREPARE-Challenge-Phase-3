...
y =  pd.read_csv('/kaggle/input/dataset/train_labels.csv')

train = pd.read_csv("/kaggle/input/dataset/train_features.csv")
test = pd.read_csv("/kaggle/input/dataset/test_features.csv")
...
data =pd.concat((train,test))
data['decis_personal_12'] = data['decis_personal_12'].replace(r).astype('float')
#for catboost we convert data into category after filling the nans with '' as catboost does not support nan in categorical features
...
###############################################################################
y=pd.read_csv("/kaggle/input/dataset/train_labels.csv")
ss = pd.read_csv("/kaggle/input/dataset/submission_format.csv")
y =pd.concat((y,ss))
...
cat_cols = [
    i for i in data_cat.columns if (
        i not in float_cols) & (
            i != 'uid') & (
                i != 'id')]
y_test = pd.read_csv("/kaggle/input/dataset/test_labels.csv")
...
# The noise is added to improve the output of QuantileTransformer.
...
data_cat.to_csv('/kaggle/working/data_cat.csv')
data_gbm.to_csv('/kaggle/working/data_gbm.csv')
a = open('/kaggle/working/data_processed_for_tabM.pkl', 'wb')
