...
data_cat  =pd.read_csv('/kaggle/working/data_cat.csv',index_col=0)
data_gbm  =pd.read_csv('/kaggle/working/data_gbm.csv',index_col=0)
y_train = pd.read_csv('/kaggle/input/dataset/train_labels.csv')['composite_score']
y_test = pd.read_csv('/kaggle/input/dataset/test_labels.csv')['composite_score']
a = open('/kaggle/working/data_processed_for_tabM.pkl', 'rb')
data_numpy = pickle.load(a)
a.close()
ids = pd.read_csv("/kaggle/input/dataset/test_labels.csv")['uid'].values
...
pd.Series(index =ids, data =pred.reshape(len(pred),), ).to_csv('/kaggle/working/predictions_Lightgbm.csv',header=False)
...
s = .25 * pred + .75 * pred2
pd.Series(index =ids, data = pred2.reshape(len(pred2),)).to_csv('/kaggle/working/predictions_Catboost.csv',header=False)

a = open('/kaggle/working/lightgbm_fitted.pkl','wb')
pickle.dump(gbm,a)
a.close()
a = open('/kaggle/working/catboost_fitted.pkl','wb')
pickle.dump(model,a)
a.close()
...
if torch.cuda.is_available():
  ...
  pd.Series(index=ids, data = prediction).to_csv('/kaggle/working/predictions_tabPFN.csv',header=False)
  a = open('/kaggle/working/tabPFN_fitted.pkl','wb')
  pickle.dump(clf,a)
  a.close()
